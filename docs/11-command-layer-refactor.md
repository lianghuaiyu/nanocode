...
| `/memory eval generate` | 596 | 精确 | system extension command → `RuntimeThread.spawn_memory_eval()` |
| `/memory optimize` | 600 | 精确 | system extension command → `RuntimeThread.run_extension_task("memory_optimize", ...)` |
| `/memory eval [...]` | 604 | 精确 OR `startswith` | `handle_eval_command` → 改 `eval_store` **模块单例** |
| `/memory` | 608 | 精确 | `RuntimeThread.memory_overview()`,只读 |
