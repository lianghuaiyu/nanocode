...
- **RPC 驱动 slash 命令**：命令层已断奶到 `RuntimeThread` 稳定 API（B-list 完成），但命令仍经
  `ui.print_*` 直接写 stdout——RPC 模式下会与 JSON 事件流串扰。要让 RPC 客户端驱动 slash 命令，
  需把命令输出也结构化（返回文本而非直接 print，或经事件流）。thread API 已就绪。
