...
class EmbeddingModel:
    ...
    def __init__(
        ...
        embed_callable: Optional[Callable[[List[str]], List[List[float]]]] = None,
        dimension: Optional[int] = None,
    ):
        # Host-injected synchronous callable takes precedence over loading any
        # local sentence-transformers model. Signature:
        ...
        if self._embed_callable is not None:
            if dimension is None:
                raise ValueError(
                    "dimension is required when embed_callable is provided "
                    "(used for the vector store table schema)."
                )
            self.model = None
            self.model_name = model_name or "host-injected"
            self.use_optimization = use_optimization
            self.dimension = dimension
            self.model_type = "host_callable"
    ...
    def encode_single(self, text: str, is_query: bool = False) -> np.ndarray:
        ...
        """
        if self._embed_callable is not None:
            # Returns a single 1-D vector (vector_store calls .tolist() on it).
            return self._embed_to_array([text])[0]
    ...
    def encode_documents(self, documents: List[str]) -> np.ndarray:
        ...
        """
        if self._embed_callable is not None:
            # Returns a 2-D array; vector_store iterates rows and calls .tolist().
            return self._embed_to_array(documents)
        return self.encode(documents, is_query=False)

    def _embed_to_array(self, texts: List[str]) -> np.ndarray:
        """Call the host-injected embed callable and return a float32 ndarray."""
        if isinstance(texts, str):
            texts = [texts]
        vectors = self._embed_callable(texts)
