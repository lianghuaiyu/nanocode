...
class LLMClient:
    ...
    def __init__(
        ...
        # Host-injected synchronous callable takes precedence over the
        # built-in OpenAI client. Signature: callable(messages) -> str.
        ...
        if self._llm_callable is not None:
            # Do not construct an OpenAI client when a callable is injected.
            self.client = None
            return

        # Lazy import so the package is usable without the openai dependency
        # when a host callable is injected.
    ...
    def chat_completion(
        ...
        """
        # Host-injected callable path: ignore response_format (downstream uses
        # extract_json as a fallback) and delegate directly.
        if self._llm_callable is not None:
