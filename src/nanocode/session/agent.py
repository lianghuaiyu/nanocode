...
class AgentSession:
    ...
    async def compact(self, instructions: str | None = None) -> None:
        ...
        if instructions:
            summary = await (a._compact_openai(prefix_messages, instructions) if a.use_openai
                             else a._compact_anthropic(prefix_messages, instructions))
        else:
    ...
    def _summary_text_for_entries(self, entries: list) -> str:
        """Plain transcript for a branch-summary LLM request."""
        parts: list[str] = []
        for e in entries:
            if e.type == _tree.MESSAGE:
    ...
    async def move_to_with_branch_summary(self, target_id: "str | None",
        ...
        """Move leaf to target, summarizing the abandoned branch as a branch_summary entry."""
        from .render import ModelCtx, render
        ...
        abandoned = self._abandoned_branch_entries(old_leaf, target_id)
        transcript = self._summary_text_for_entries(abandoned)
        if not transcript:
            return self.move_to(target_id)

        prompt = ("Summarize the abandoned branch so the conversation can continue from a different "
                  "point without replaying that branch. Preserve decisions, file paths, commands, "
                  "tool results, and unresolved work.")
        if focus:
            prompt += f"\n\nCustom focus instructions: {focus}"
        ...
        data = {"fromId": old_leaf, "summary": summary}
        if focus:
            data["details"] = {"focus": focus}
        mgr.append(_tree.BRANCH_SUMMARY, data, parent_id=target_id)
