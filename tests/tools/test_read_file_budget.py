...
def test_offset_limit_window(tmp_path):
    ...
    out = read_file.run({"file_path": str(p), "offset": 50, "limit": 10})
...
def test_offset_to_end_shows_range_note(tmp_path):
    ...
    out = read_file.run({"file_path": str(p), "offset": 8})
...
def test_missing_file_errors(tmp_path):
    out = read_file.run({"file_path": str(tmp_path / "nope.txt")})
...
def test_invalid_offset_limit_fall_back_to_defaults(tmp_path):
    p = tmp_path / "a.txt"
    p.write_text("a\nb\nc")
    out = read_file.run({"file_path": str(p), "offset": -3, "limit": "bad"})
