"""Tests for the REPL `/memory eval` command handler (cli.handle_eval_command).
...
class TestHandleEvalCommand:
    def test_pending_empty(self):
        out = cli.handle_eval_command("pending")
    ...
    def test_confirm_moves_candidate(self):
        ...
        out = cli.handle_eval_command(f"confirm {cid}")
        assert cid in out
    ...
    def test_reject_moves_candidate(self):
        cid = eval_store.add_pending(_candidate())
        out = cli.handle_eval_command(f"reject {cid}")
        assert cid in out
    ...
    def test_rejected_list(self):
        ...
        out = cli.handle_eval_command("rejected")
        assert cid in out
        assert "Q-rej?" in out

    def test_unknown_subcommand_shows_usage(self):
        out = cli.handle_eval_command("frobnicate")
        assert "usage" in out.lower()
