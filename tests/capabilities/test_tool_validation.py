...
def _validate(name: str, inp: dict) -> str | None:
    return validate_tool_input(name, inp, registry=REGISTRY)


# ─── 隐藏字段（下划线键）一律拒 ─────────────────────────────────

def test_rejects_cwd_spoof():
    err = _validate("run_shell", {"command": "pwd", "_cwd": "/"})
    assert err is not None and "_cwd" in err


def test_rejects_session_id_spoof():
    err = _validate("run_shell", {"command": "pwd", "_session_id": "evil"})
...
def test_rejects_underscore_key_even_for_unknown_tool():
    ...
    assert _validate("mcp__server__tool", {"_cwd": "/"}) is not None


# ─── unknown / 已删参数拒（additionalProperties:false）────────────

def test_rejects_unknown_key():
    assert _validate("run_shell", {"command": "pwd", "mount_workspace": True}) is not None
    assert _validate("run_shell", {"command": "pwd", "network": "public"}) is not None


def test_rejects_removed_list_files_pattern_key():
    assert _validate("list_files", {"path": ".", "pattern": "src/**/*"}) is not None


def test_rejects_missing_required():
    assert _validate("run_shell", {}) is not None
    assert _validate("read_file", {}) is not None


def test_rejects_type_mismatch():
    assert _validate("run_shell", {"command": 123}) is not None
    assert _validate("run_shell", {"command": "pwd", "run_in_background": "yes"}) is not None


# ─── 合法输入通过 ────────────────────────────────────────────────

def test_accepts_valid_run_shell():
    assert _validate("run_shell", {"command": "pwd"}) is None
    assert _validate(
...
def test_accepts_valid_read_file():
    assert _validate("read_file", {"file_path": "/x", "offset": 1, "limit": 5}) is None


def test_unknown_tool_without_underscore_passes():
    # 无本地 schema 的 MCP 工具，普通键放行（其 schema 由 MCP server 把关）。
    assert _validate("mcp__server__tool", {"anything": 1}) is None
...
# required + additionalProperties:false + typed properties，本地校验也**不**二次否决；这些规则
# 由 MCP server 自己把关，nanocode 本地仅保留下划线键守卫。
