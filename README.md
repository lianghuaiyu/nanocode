...
- **会话与子 Agent 产物**：每轮对话自动保存；session v2 把主 Agent 与每个子 Agent 的轨迹写到 `~/.nanocode/sessions/<id>/agents/<id>/`（`meta.json` / `prompt.txt` / `result.md` / `wire.jsonl` 事件流 / `messages.json`），可恢复、可 replay、可审计。`wire.jsonl` 始终开启，与可选的 `--trace` 调试 trace 解耦。
...
├── subagents/              # 子 Agent：config（类型/manifest 解析 + extends + 信任闸）/ result（结构化 AgentResult 解析）/ prompts
├── mcp/                    # MCP：connection（单连接）/ manager（多服务器路由）
├── session/                # 会话持久化：store（flat）/ v2（按 Agent 的目录产物 + wire.jsonl）
├── tasks/                  # 后台任务：manager / runner / models / inject（完成回注）
├── trace/                  # 事件流：tracer / sinks（JsonlSink）/ config
...
| `subagents/` | 子 Agent 类型/manifest 解析、`extends` 收窄、有效工具集与项目信任闸；结构化 AgentResult 解析 |
| `mcp/` | MCP stdio 连接、工具发现与前缀路由 |
| `session/` | 会话保存/恢复/列举；session v2 按 Agent 写 meta/prompt/result/wire.jsonl/messages |
